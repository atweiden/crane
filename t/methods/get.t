...
subtest
{
    ...
    throws-like {Crane.get(%data, :path(qw<legumes 99>))}, X::Crane::PathDNE,
        'Get operation on nonexistent path fails';
    throws-like {Crane.get(%data, :path(qw<legumes 0 bar>))}, X::Crane::PathDNE,
        'Get operation on nonexistent path fails';
    ...
    is-deeply Crane.get(%data, :path(['legumes']), :p), %data<legumes>:!p,
        'Is expected pair';
    is-deeply Crane.get(%data, :path(qw<legumes 0>)), %data<legumes>[0],
        'Is expected value';
    is-deeply Crane.get(%data, :path(qw<legumes 0>), :k), %data<legumes>[0]:!k,
        'Is expected key';
    is-deeply Crane.get(%data, :path(qw<legumes 0>), :p), %data<legumes>[0]:!p,
        'Is expected pair';
    is Crane.get(%data, :path(qw<legumes 0 instock>)), 4,
        'Is expected value';
    is Crane.get(%data, :path(qw<legumes 0 instock>), :k), 'instock',
        'Is expected key';
    is Crane.get(%data, :path(qw<legumes 0 instock>), :p), {:instock(4)},
        'Is expected pair';
    is Crane.get(%data, :path(qw<legumes 0 name>)), 'pinto beans',
        'Is expected value';
    is Crane.get(%data, :path(qw<legumes 0 name>), :k), 'name',
        'Is expected key';
    is Crane.get(%data, :path(qw<legumes 0 name>), :p), {:name<pinto beans>},
        'Is expected pair';
    is Crane.get(%data, :path(qw<legumes 0 unit>)), 'lbs',
        'Is expected value';
    is Crane.get(%data, :path(qw<legumes 0 unit>), :k), 'unit',
        'Is expected key';
    is Crane.get(%data, :path(qw<legumes 0 unit>), :p), {:unit<lbs>},
        'Is expected pair';
